...
function generatePrintHTML(sections, recipes) {
  ...
  return `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>My Cookbook</title><style>*{box-sizing:border-box;margin:0;padding:0;}body{font-family:'Trebuchet MS','Gill Sans',sans-serif;background:#fff;color:#1A1208;font-size:11pt;line-height:1.5;}.cover{page-break-after:always;height:100vh;display:flex;flex-direction:column;align-items:center;justify-content:center;background:#6B4C3B;color:#F5F0E8;text-align:center;padding:3rem;}.cover-star{font-size:4rem;color:#C4A882;margin-bottom:1rem;}.cover-title{font-family:Georgia,serif;font-size:3rem;font-weight:700;letter-spacing:.05em;}.cover-sub{font-size:.85rem;letter-spacing:.2em;text-transform:uppercase;margin-top:.5rem;color:#C4A882;}.toc-page{page-break-after:always;padding:2.5rem;background:#F5F0E8;min-height:100vh;background-image:repeating-linear-gradient(to bottom,transparent,transparent 29px,#E2D9C8 29px,#E2D9C8 30px);background-position-y:36px;}.toc-heading{font-family:Georgia,serif;font-size:1.6rem;font-weight:700;color:#1A1208;margin-bottom:1.5rem;padding-bottom:.5rem;border-bottom:2px solid #9B7355;}.toc-sec{margin-bottom:1.2rem;}.toc-sec-name{font-weight:700;font-size:.82rem;text-transform:uppercase;letter-spacing:.1em;color:#5C3D8F;padding:.3rem 0;border-bottom:1px solid #E2D9C8;margin-bottom:.3rem;}.toc-row{padding:.18rem 0 .18rem 1rem;font-size:.88rem;color:#3D2E1A;}.toc-num{color:#9B7355;font-weight:600;}.sec-div{page-break-before:always;page-break-after:always;height:100vh;display:flex;align-items:center;justify-content:center;background:#6B4C3B;color:#F5F0E8;}.sec-div-inner{text-align:center;}.sec-div-lbl{font-size:.75rem;text-transform:uppercase;letter-spacing:.2em;color:#C4A882;margin-bottom:.5rem;}.sec-div-name{font-family:Georgia,serif;font-size:2.8rem;font-weight:700;}.recipe-page{page-break-before:always;padding:2rem 2.5rem;background:#FDFAF4;background-image:repeating-linear-gradient(to bottom,transparent,transparent 29px,#E2D9C8 29px,#E2D9C8 30px);background-position-y:36px;min-height:100vh;}.recipe-header{padding-bottom:.75rem;margin-bottom:1rem;border-bottom:2px solid #C4A882;}.recipe-title{font-family:Georgia,serif;font-size:1.8rem;font-weight:700;color:#1A1208;line-height:1.2;margin-bottom:.4rem;}.recipe-meta{display:flex;flex-wrap:wrap;gap:1rem;font-size:.78rem;color:#7A6548;}.recipe-meta .src{font-style:italic;}.block{margin-bottom:1.2rem;}.block h3{font-size:.68rem;text-transform:uppercase;letter-spacing:.12em;color:#9B7355;font-weight:700;margin-bottom:.5rem;padding-bottom:.2rem;border-bottom:1px solid #E2D9C8;}.ing-grid{display:grid;grid-template-columns:1fr 1fr;gap:.12rem 1.5rem;font-size:.88rem;}.ing{color:#3D2E1A;padding:.1rem 0;}ol{padding-left:1.2rem;}ol li{margin-bottom:.45rem;font-size:.9rem;line-height:1.55;color:#3D2E1A;}.notes-block{background:#FAF6EE;border-left:3px solid #C4A882;padding:.6rem .85rem;border-radius:0 3px 3px 0;}.notes-text{font-style:italic;font-size:.85rem;color:#7A6548;line-height:1.5;}@media print{body{print-color-adjust:exact;-webkit-print-color-adjust:exact;}.cover,.sec-div{background:#6B4C3B!important;}}</style></head><body><div class="cover"><div class="cover-star">★</div><div class="cover-title">My Cookbook</div><div class="cover-sub">A personal collection</div></div><div class="toc-page"><div class="toc-heading">★ Table of Contents</div>${sorted.map(tocSec).join("")}</div>${sorted.map(sec => { const recs = recipes.filter(r => r.section_id === sec.id); if (!recs.length) return ""; return `<div class="sec-div"><div class="sec-div-inner"><div class="sec-div-lbl">Section</div><div class="sec-div-name">${sec.name}</div></div></div>${recs.map(recipeHTML).join("")}`; }).join("")}</body></html>`;
}
        ...
        <div style={{ display: "flex", flex: 1, minHeight: 0 }}>
          ...
          <div style={{ flex: 1, overflowY: "auto", background: C.pageInner, minWidth: 0 }}>
            {(activeTab === "RECIPES" || nav?.recipe || nav?.section) && (
              <div style={{ padding: "0.85rem 0.85rem 5rem", height: "100%" }}>
                {nav?.recipe ? (
                  ...
                  <>
                    ...
                    {search ? (
                        ...
                        {searched.map(r => (
                          <button key={r.id} onClick={() => goNav({ section: sections.find(s => s.id === r.section_id), recipe: r })}
                            ...
                            <div>
                              ...
                              <div style={{ fontSize: "0.68rem", color: C.inkMuted }}>{sections.find(s => s.id === r.section_id)?.name}</div>
                            </div>
                      ...
                      <>
                        {loading && <div style={{ color: C.inkFaint, textAlign: "center", padding: "2rem" }}>Opening cookbook…</div>}
                        {sections.map(s => (
            ...
            {["CONVERSIONS","TIPS","KITCHEN NOTES","PLANNER"].includes(activeTab) && (
              <div style={{ display: "flex", alignItems: "center", justifyContent: "center", height: "100%", flexDirection: "column", color: C.inkFaint }}>
                <div style={{ fontSize: "1.8rem" }}>📄</div>
                <div>{activeTab} Coming soon</div>
              </div>
            )}
          </div>
          ...
          <div style={{ width: 36, flexShrink: 0, background: C.spine, display: "flex", flexDirection: "column", paddingTop: "0.5rem", paddingBottom: "0.5rem", gap: 3 }}>
            {FIXED_TABS.map(tab => {
              ...
              return (
                <button key={tab} onClick={() => { setActiveTab(tab); setNav(null); }}
                  style={{ flex: 1, background: active ? C.pageInner : "#4A2A6A", border: "none", borderRadius: "0 6px 6px 0", borderLeft: active ? `3px solid ${C.accent}` : "3px solid transparent", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>
                  <span style={{ writingMode: "vertical-rl", transform: "rotate(180deg)", fontSize: "0.48rem", fontFamily: C.fontSans, fontWeight: "700", color: active ? C.accent : "rgba(220,205,255,0.85)", textTransform: "uppercase" }}>{tab}</span>
                </button>
              );
            })}
            <div style={{ marginTop: "auto", paddingTop: "0.3rem", display: "flex", flexDirection: "column", alignItems: "center", gap: 2 }}>
              <button onClick={() => { const name = prompt("New section name:"); if (name?.trim()) { const s = { id: `sec-${Date.now()}`, user_id: authUser.id, name: name.trim(), position: sections.length }; dbUpsertSection(s).then(saved => setSections(prev => [...prev, saved || s])); } }}
                style={{ width: 26, height: 26, borderRadius: "50%", background: C.accentFade, border: `2px solid ${C.accentLight}`, color: C.accent, fontSize: "1rem", fontWeight: "bold", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>+</button>
              {/* ✅ FIXED HYDRATION SPAN */}
              <span style={{ fontSize: "0.38rem", color: "rgba(220,205,255,0.7)", fontFamily: C.fontSans, textTransform: "uppercase", letterSpacing: "0.06em", textAlign: "center", lineHeight: 1.3, display: "block", whiteSpace: "pre-line" }}>{"Add\nNew\nSection"}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      {!nav?.recipe && !nav?.section && (activeTab === "RECIPES" || activeTab === "DASHBOARD") && (
        <div style={{ position: "fixed", bottom: "1.2rem", right: "3rem", display: "flex", flexDirection: "column", alignItems: "flex-end", gap: "0.5rem", zIndex: 50 }}>
          <button onClick={() => { setActiveTab("RECIPES"); setRecipeModal({}); }} disabled={sections.length === 0}
            style={{ background: C.accent, border: "none", borderRadius: 22, color: "#fff", padding: "0.55rem 1.1rem", fontSize: "0.82rem", fontWeight: "700", cursor: "pointer", boxShadow: "0 3px 12px rgba(92,61,143,0.5)" }}>
            + Add Recipe
          </button>
          <div style={{ display: "flex", gap: "0.35rem", alignItems: "center", background: C.pageInner, border: `1px solid ${C.spineFaint}`, borderRadius: 22, padding: "0.38rem 0.5rem 0.38rem 0.85rem" }}>
            <input value={addSecName} onChange={e => setAddSecName(e.target.value)} onKeyDown={e => e.key === "Enter" && addSection()} placeholder="New section…"
              style={{ background: "transparent", border: "none", outline: "none", color: C.ink, fontSize: "0.78rem", width: 110 }} />
            <button onClick={addSection} disabled={!addSecName.trim()} style={{ background: C.accent, border: "none", borderRadius: 18, color: "#fff", padding: "0.28rem 0.65rem", fontSize: "0.75rem", fontWeight: "bold" }}>+ Section</button>
          </div>
        </div>
      )}

      {recipeModal !== null && (
        <RecipeModal recipe={recipeModal?.id ? recipeModal : null} sections={sections} defaultSectionId={recipeModal?._defaultSection || nav?.section?.id || sections[0]?.id} onSave={saveRecipe} onDelete={deleteRecipe} onClose={() => setRecipeModal(null)} />
      )}
